...
- 修复快路径提取在部分压缩记录上的误判失败：当压缩记录解压失败时，改为跳过该记录扫描，而不是继续按压缩字节解析子记录导致 `Invalid file format`
- 影响范围：修复 `extract_strings_from_file` 默认快路径与 CLI 默认快路径在特定插件上的失败问题（如 DynDOLOD / Dawnguard / Whiterun Has Walls 场景）

### 测试

- 新增回归测试，覆盖“无效压缩记录应被跳过且不应使整次快路径提取失败”
- 复测快路径与 legacy 路径提取结果一致（修复案例）
